...
// void init(stack **s) {
//   *s = calloc(1, sizeof(stack)); // `calloc` instead of `malloc` to initialize `s->size` to 0
// }

stack *init() {
  stack *s = calloc(1, sizeof(stack)); // `calloc` instead of `malloc` to initialize `s->size` to 0
}
